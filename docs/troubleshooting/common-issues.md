...
# Check events
kubectl get events -n dclaw-api --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-api deployment/dclaw-api-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-api-db-credentials -n dclaw-api
   ```
